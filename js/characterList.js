...
function renderList(list, total) {
  ...
  if (!list.length) {
    ...
    dataUnavailable.textContent =
      "Your favorites list is waiting for its first Superhero!";
    if (total == 0) {
...
function addToFavourites(characterId) {
  if (!favouriteCharacters.includes(characterId)) {
    ...
    favouriteCharacters = favouriteCharacters.filter((favouriteId) => {
      return favouriteId != characterId;
    });
  }
